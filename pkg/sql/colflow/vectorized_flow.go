...
var VectorizeTestingBatchSize = settings.RegisterValidatedIntSetting(
	...
	fmt.Sprintf("the size of a batch of rows in the vectorized engine. 0 uses the default batch size. Must be a value less than or equal to %d", coldata.MaxBatchSize),
	0,
	func(newBatchSize int64) error {
		if newBatchSize > coldata.MaxBatchSize {
			return errors.Errorf("batch size %d may not be larger than %d", newBatchSize, coldata.MaxBatchSize)
		}
		return nil
	},
)
...
func (s *vectorizedFlowCreator) setupFlow(
	...
	for len(queue) > 0 {
		...
		if (flowCtx.EvalCtx.SessionData.VectorizeMode == sessiondata.Vectorize192Auto) &&
			...
			return nil, errors.Errorf("hash router encountered when vectorize=192auto")
		}
